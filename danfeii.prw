...
//INICIO CUSTOMIZAÇÃO
//#DEFINE MAXITEMC   038  - aumentado de 38 caracteres  para 52 car.  // Máxima de caracteres por linha de produtos/serviços
#DEFINE MAXITEMC   065                                                // Máxima de caracteres por linha de produtos/serviços
...
//	aumentado de 09 caracteres  para 11 caracteres
...
//FIM CUSTOMIZAÇÃO
//ERPSERV - FONTE UTILIZADO : 29/12/2018 09:08:08
/*/
...
/*/
//CUSTOMIZAÇÃO AIRBP - parâmetro lJobAirBP
User Function PrtNfeSef(cIdEnt, cVal1		, cVal2		, oDanfe,;
						oSetup, cFilePrint	, lIsLoja	, nTipo, lJobAirBP	)
...
Default nTipo	:= 0
Default lJobAirBP	:= .F. //CUSTOMIZAÇÃO AIRBP - execução de job AIRBP
...
If nTipo <> 1
	...
	If lPergunte
		//Customização AIRBP
		RPTStatus( {|lEnd| U_DANFEProc(@oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja, ,lJobAirBP)}, "Imprimindo DANFE..." )
	EndIf
...
ElseIf nTipo == 1
	...
	If lPergunte
		RPTStatus( {|lEnd| U_DANFEProc(@oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja, nTipo, lJobAirBP)}, "Imprimindo DANFE..." )
	EndIf
EndIf
...
User Function DANFEProc(	oDanfe	, lEnd		, cIdEnt	, cVal1,;
							cVal2	, lExistNfe	, lIsLoja	, nTipo, lJobAirBP)
...
Local cDirPDF	 := cDirPai + GetNewPar("MV_XDIRPDF")
Local cDirXML	 := cDirPai + GetNewPar("MV_XDIRXML")
Local _lNFe		 := .T.
local lChave	:= .F.
Local cChavSF3	:= ""
Local nTotalReg := 0
	...
	#IFDEF TOP
		...
		If MV_PAR04==1
		 	...
		 	If lSdoc
				...
				cWhere := "%SubString(SF3.F3_CFO,1,1) < '5' AND SF3.F3_FORMUL='S' AND SF3.F3_SDOC = '"+ cSerie + "' " //Customização AIRBP //"' AND SF3.F3_ESPECIE = 'SPED'"
			Else
				cSerie := Padr(MV_PAR03,TamSx3("F3_SERIE")[1])
				cWhere := "%SubString(SF3.F3_CFO,1,1) < '5' AND SF3.F3_FORMUL='S' AND SF3.F3_SERIE = '"+ cSerie + "' "//Customização AIRBP //"' AND SF3.F3_ESPECIE = 'SPED'"
			Endif
		...
		ElseIf MV_PAR04==2
		 	...
		 	If lSdoc
				cCampos += ", SF3.F3_SDOC"
				cSerie := Padr(MV_PAR03,TamSx3("F3_SDOC")[1])
				cWhere := "%SubString(SF3.F3_CFO,1,1) >= '5' AND SF3.F3_SDOC = '"+ cSerie +  "' " //Customização AIRBP //"' AND SF3.F3_ESPECIE = 'SPED'"
			Else
				cSerie := Padr(MV_PAR03,TamSx3("F3_SERIE")[1])
				cWhere := "%SubString(SF3.F3_CFO,1,1) >= '5' AND SF3.F3_SERIE = '"+ cSerie +  "' " //Customização AIRBP //"' AND SF3.F3_ESPECIE = 'SPED'"
			Endif
		Else
			...
			If lSdoc
				cCampos += ", SF3.F3_SDOC"
				cSerie := Padr(MV_PAR03,TamSx3("F3_SDOC")[1])
				cWhere := "%SF3.F3_SDOC = '"+ cSerie +  "' " //Customização AIRBP //"' AND SF3.F3_ESPECIE = 'SPED'"
			Else
				cSerie := Padr(MV_PAR03,TamSx3("F3_SERIE")[1])
				cWhere := "%SF3.F3_SERIE = '"+ cSerie + "' " //Customização AIRBP // "' AND SF3.F3_ESPECIE = 'SPED'"
			Endif
				...
				If !Empty(aXML[nX][2])
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						If Empty(cAviso) .And. Empty(cErro)
							//CUSTOMIZAÇÃO AIRBP - Alfa - Só considerar objetos do tipo NF-e
							...
							EndIf
						EndIf
						//INICIO CUSTOMIZAÇÃO AIRBP
						If lJobAirBP .And. _lNFe

							// Diretório temporário para geração da DANFE.
							If !ExistDir("c:\temp\")
								MakeDir("c:\temp\")
							EndIf
							...
							oDanfe:cPathPDF := "c:\temp\"
  							oDanfe:cFileName := Upper(oDanfe:cFileName)
  							oDanfe:Print() // Cria o PDF
							  
							__cFilCrea := Upper(oDanfe:cPathPDF + StrTran(oDanfe:cFileName,".REL",".PDF"))
  							//__cFilCrea := Upper(oDanfe:cPathPDF + oDanfe:cFileName)
								
  							If File(__cFilCrea)     
  								
  								_cSerNota := aNotas[nX,5] + "_" + cFilAnt + "_" + aNotas[nX,6] + "_" + aNotas[nX,7]
								//_cSerNota := aNotas[nX,5] + cFilAnt + aNotas[nX,6] + aNotas[nX,7]
  									                                 
  								//Numero + Serie + CodCliente + Loja
 		 						__cFilPrin := Upper(_cSerNota) + ".pdf"

								If !File(oDanfe:cPathPDF + __cFilPrin)
	 		 						FRename(__cFilCrea, oDanfe:cPathPDF + __cFilPrin)
 		 						EndIf 

	 		 					//FRename(__cFilCrea, oDanfe:cPathPDF + __cFilPrin)                              
								
								//Copia a danfe para o servidor
 		 						//CpyT2S(oDanfe:cPathPDF + __cFilPrin, cDirPDF, .T.)
								__CopyFile( oDanfe:cPathPDF + __cFilPrin, cDirPDF + __cFilPrin )

								FErase(__cFilCrea)

 		 						// Pastas conforme o ano e o mes
 		 						lGrvFiles := .T.
									
								// Gravacao do arquivo XML
								_aNotas := {}
								aXml2	:= {}
									
								Aadd(_aNotas,{} )
								Aadd( Atail(_aNotas),.F.)
								Aadd( Atail(_aNotas),Iif(SF3->F3_CFO < "5","E","S"))
								Aadd( Atail(_aNotas),SF3->F3_ENTRADA)
								Aadd( Atail(_aNotas),SF3->F3_SERIE)
								Aadd( Atail(_aNotas),SF3->F3_NFISCAL)
								Aadd( Atail(_aNotas),SF3->F3_CLIEFOR)
								Aadd( Atail(_aNotas),SF3->F3_LOJA)
									
								aXml2 := GetXMLNFE(cIdEnt, _aNotas, @cModalidade, "" /*Iif(lCTE,"57","")*/ )
									
								If Type("aXml2") == "A" .And. Len(aXml2) > 0 
									
									cDetXml := aXml2[1,2] // XML da nota
									cFileXml := cDirXML + aNotas[nX,5] + "_" + cFilAnt + "_" + aNotas[nX,6] + "_" + aNotas[nX,7] + ".xml"
									
									cFileXml := Upper(cFileXml)

									If File(cFileXML) // Se ja tem o arquivo TXT com o XML, apaga e refaz...	
										fErase(cFileXML) 
									EndIf
										
									nHdl := fCreate(cFileXml) // Cria arquivo txt
									fWrite(nHdl,cDetXml,Len(cDetXml))
									fClose(nHdl) // Finaliza processo e fecha arquivo txt
									
								EndIf
  							EndIf
						EndIf
						//FIM CUSTOMIZAÇÃO AIRBP
						oNfe     := nil
						oNfeDPEC := nil
	...
	While  !lBreak  .And. nLenarray >= 0
			...
			If MV_PAR04==1
				...
				cFrom	:=	"%"+RetSqlName("SF1")+" SF1 %"
			 	...
			 	If lSdoc
					...
					cWhere := "%SF1.D_E_L_E_T_= '' AND SF1.F1_FILIAL ='"+xFilial("SF1")+"' AND SF1.F1_DOC <='"+MV_PAR02+ "' AND SF1.F1_DOC >='" + MV_PAR01 + "' AND SF1.F1_SDOC ='"+ cSerie + "' "//CUSTOMIZAÇÃO AIRBP //"' AND SF1.F1_ESPECIE = 'SPED' AND SF1.F1_FORMUL = 'S' ORDER BY SF1.F1_DOC%"
				Else
					cCampos += "%SF1.F1_FILIAL FILIAL, SF1.F1_DOC DOC, SF1.F1_SERIE SERIE%"
					cSerie := Padr(MV_PAR03,TamSx3("F2_SERIE")[1])
					cWhere := "%SF1.D_E_L_E_T_= '' AND SF1.F1_FILIAL ='"+xFilial("SF1")+"' AND SF1.F1_DOC <='"+MV_PAR02+ "' AND SF1.F1_DOC >='" + MV_PAR01 + "' AND SF1.F1_SERIE ='"+ cSerie + "' "//CUSTOMIZAÇÃO AIRBP //"' AND SF1.F1_ESPECIE = 'SPED' AND SF1.F1_FORMUL = 'S' ORDER BY SF1.F1_DOC%"
				Endif
			...
			ElseIf MV_PAR04==2
				...
				cFrom	:=	"%"+RetSqlName("SF2")+" SF2 %"
			 	...
			 	If lSdoc
					...
					cWhere := "%SF2.D_E_L_E_T_= '' AND SF2.F2_FILIAL ='"+xFilial("SF2")+"' AND SF2.F2_DOC <='"+MV_PAR02+ "' AND SF2.F2_DOC >='" + MV_PAR01 + "' AND SF2.F2_SDOC ='"+ cSerie + "' "//CUSTOMIZAÇÃO AIRBP //"' AND SF2.F2_ESPECIE = 'SPED' ORDER BY SF2.F2_DOC%"
				Else
					cCampos += "%SF2.F2_FILIAL FILIAL, SF2.F2_DOC DOC, SF2.F2_SERIE SERIE%"
					cSerie := Padr(MV_PAR03,TamSx3("F2_SERIE")[1])
					cWhere := "%SF2.D_E_L_E_T_= '' AND SF2.F2_FILIAL ='"+xFilial("SF2")+"' AND SF2.F2_DOC <='"+MV_PAR02+ "' AND SF2.F2_DOC >='" + MV_PAR01 + "' AND SF2.F2_SERIE ='"+ cSerie + "' "//CUSTOMIZAÇÃO AIRBP //"' AND SF2.F2_ESPECIE = 'SPED' AND SF2.F2_EMISSAO >= '" + %exp:DtoS(MV_PAR07)% + "' AND SF2.F2_EMISSAO <= '" + %exp:DtoS(MV_PAR08)% + "' ORDER BY SF2.F2_DOC%"					
				Endif
		...
		While !Eof() .And. !lBreak .And. ;
			...
			For nx:=1 To 20
				...
				aadd(Atail(aNotas),"")
				aadd(Atail(aNotas),"")
				If (nCursor >= nTotalReg) //.or. (cAliasSFX)->(Eof ()) //CUSTOMIZAÇÃO AIRBP
					lBreak :=.T.
					nx:=20
				EndIF
				nCursor++
...
For nZ := 1 To nLenDet
    ...
    Else
		...
		nVUnit   := Round(Val(oDet[nX]:_Prod:_vUnCom:TEXT),4)  // Air BP - Tratamento para DANFE imprimir a unidade comercial do produto e não a tributária [Alteração para solucionar item 1017 da lista de pendencias
	EndIf

	nQtd     	:= Val(oDet[nX]:_Prod:_qCom:TEXT)  // Air BP - Tratamento para DANFE imprimir a unidade comercial do produto e não a tributária.
	nBaseICM 	:= 0
	nValICM  	:= 0
	nValIPI  	:= 0
	nPICM    	:= 0
	...
	//INICIO CUSTOMIZAÇÃO AIRBP - Tratamento para DANFE imprimir a unidade comercial do produto e não a tributária - INÍCIO.
	cUnCom    := oDet[nX]:_Prod:_uCom:TEXT
	nQtdCom   := Val(oDet[nX]:_Prod:_qCom:TEXT)
//	nVUnitCom := Val(oDet[nX]:_Prod:_vUnCom:TEXT)
	nVUnitCom := Round(Val(oDet[nX]:_Prod:_vUnCom:TEXT),4) //  [Alteração para solucionar item 1017 da lista de pendencias
...
*/// FIM CUSTOMIZAÇÃO AIRBP 
	//INICIO CUSTOMIZAÇÃO AIRBP - Tratamento para DANFE imprimir a unidade comercial do produto e não a tributária - FIM.

	// Tratamento somente para o estado de MG, para totalizar por CFOP conforme RICMS-MG
	If lUf_MG
		// Air BP - Tratamento para DANFE imprimir a unidade comercial do produto e não a tributária - INÍCIO.
		cUnCom    := oDet[nX]:_Prod:_uCom:TEXT
		nQtdCom   := Val(oDet[nX]:_Prod:_qCom:TEXT)
//		nVUnitCom := Val(oDet[nX]:_Prod:_vUnCom:TEXT)
		nVUnitCom := Round(Val(oDet[nX]:_Prod:_vUnCom:TEXT),4) //  [Alteração para solucionar item 1017 da lista de pendencias
...
*/
	//FIM CUSTOMIZAÇÃO AIRBP  - Tratamento para DANFE imprimir a unidade comercial do produto e não a tributária - FIM.
	Endif
...
//	    nVUnitTrib	:= Val(oDet[nX]:_Prod:_vUnTrib:TEXT)
        nVUnitTrib  := Round(Val(oDet[nX]:_Prod:_vUnCom:TEXT),4) // CUSTOMIZAÇÃO AIRBP - [Alteração para solucionar item 1017 da lista de pendencias
		...
		/***INICIO CUSTOMIZACAO AIRBP RETIRA A SEGUNDA UNIDADE DE MEDIDA DO DANFE
			/*aadd(aItens,{;
				"",;
				"",;
				"",;
...
If nfolha==1
	If lMv_Logod
		oDanfe:SayBitmap(042/*045*/,000/*001*/,cLogoD,095/*080*/,096/*090*/)
	Else
		oDanfe:SayBitmap(042/*045*/,000/*001*/,cLogo,095/*080*/,096/*090*/)
	EndIF
Endif
...
/*ElseIf cModFrete =="3" //Customização AIRBP
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > nMaxItemP2
			...
			If lMv_Logod
				oDanfe:SayBitmap(/*002*/000+nLinhavers,/*003*/000,cLogoD,095/*085*/,096/*085*/)
			Else
				oDanfe:SayBitmap(/*002*/000+nLinhavers,/*003*/000,cLogo,095/*085*/,096/*085*/)
			EndIf
	...
	Else
		...
		oDanfe:Say(nLinha, nAuxH2 +4, aAux[1][7][nY], oFont08:oFont) // QUANT - CUSTOMIZAÇÃO AIRBP
		nAuxH += aTamCol[7]

		nAuxH2 := len(aAux[1][8][nY]) + (nAuxH + (aTamCol[8]) - RetTamTex(aAux[1][8][nY], oFont08:oFont, oDanfe))
		oDanfe:Say(nLinha, nAuxH2 -3, aAux[1][8][nY], oFont08:oFont) // V UNITARIO - CUSTOMIZAÇÃO AIRBP
		nAuxH += aTamCol[8]

		nAuxH2 := len(aAux[1][9][nY]) + (nAuxH + (aTamCol[9]) - RetTamTex(aAux[1][9][nY], oFont08:oFont, oDanfe))
		oDanfe:Say(nLinha, nAuxH2 +2, aAux[1][9][nY], oFont08:oFont) // V. TOTAL - CUSTOMIZAÇÃO AIRBP
		nAuxH += aTamCol[9]
...
For nZ := 1 To nLenDet
	...
	nVUnit	 := Round(Val(oDet[nX]:_Prod:_vUnCom:TEXT),4) //CUSTOMIZAÇÃO AIRBP -  [Alteração para solucionar item 1017 da lista de pendencias
...
*/	
	// Air BP - Tratamento para DANFE imprimir a unidade comercial do produto e não a tributária - FIM.
	cAux     := AllTrim(SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte),(nMaxDes+1)))
...
If MsSeek(xFilial("SF2")+cSeek)
	If SF2->(FieldPos("F2_HORA"))<>0 .And. ( Empty(SF2->F2_HORA) .Or. SF2->F2_HORA <> cDtHrRec ) //CUSTOMIZAÇÃO AIRBP
		RecLock("SF2")
...
If MsSeek(xFilial("SF1")+cSeek)
	If SF1->(FieldPos("F1_HORA"))<>0 .And. ( Empty(SF1->F1_HORA) .Or. SF1->F1_HORA <> cDtHrRec ) //CUSTOMIZAÇÃO AIRBP
		RecLock("SF1")
