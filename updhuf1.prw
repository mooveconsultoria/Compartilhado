...
/*/
//--------------------------------------------------------------------
User Function UPDHUF1( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPINTGER" )
				Else
					MsgStop( "Atualização não Realizada.", "UPINTGER" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZA1'																	, .T. }, ; //X3_ARQUIVO
	{ '20'																	, .T. }, ; //X3_ORDEM
	{ 'ZA1_TXHUF1'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 14																	, .T. }, ; //X3_TAMANHO
	{ 2																		, .T. }, ; //X3_DECIMAL
	{ 'Tx Hookup 1'															, .T. }, ; //X3_TITULO
	{ 'Tx Hookup 1'															, .T. }, ; //X3_TITSPA
	{ 'Tx Hookup 1'															, .T. }, ; //X3_TITENG
	{ 'Taxa HookupFee para moeda Real'										, .T. }, ; //X3_DESCRIC
	{ 'Taxa HookupFee para moeda Real'										, .T. }, ; //X3_DESCSPA
	{ 'Taxa HookupFee para moeda Real'										, .T. }, ; //X3_DESCENG
...
aAdd( aSX3, { ;
	{ 'ZA1'																	, .T. }, ; //X3_ARQUIVO
	{ '21'																	, .T. }, ; //X3_ORDEM
	{ 'ZA1_TXHUF2'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 14																	, .T. }, ; //X3_TAMANHO
	{ 2																		, .T. }, ; //X3_DECIMAL
	{ 'Tx Hookup 2'															, .T. }, ; //X3_TITULO
	{ 'Tx Hookup 2'															, .T. }, ; //X3_TITSPA
	{ 'Tx Hookup 2'															, .T. }, ; //X3_TITENG
	{ 'Taxa HookupFee para moeda Dolar'										, .T. }, ; //X3_DESCRIC
	{ 'Taxa HookupFee para moeda Dolar'										, .T. }, ; //X3_DESCSPA
	{ 'Taxa HookupFee para moeda Dolar'										, .T. }, ; //X3_DESCENG
	{ '@E 99,999,999,999.99'												, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
aAdd( aSX6, { ;
	...
	'MV_FRETMOE'															, ; //X6_VAR
	'C'																		, ; //X6_TIPO
	'Respeita a moeda do P.Venda para Frete/Seguro ?   '					, ; //X6_DESCRIC
	'¿Respeta moneda del precio de venta para flete/   '					, ; //X6_DSCSPA
	'Consider the currency of the sales order for      '					, ; //X6_DSCENG
	''																		, ; //X6_DESC1
	''																		, ; //X6_DSCSPA1
	''																		, ; //X6_DSCENG1
	''																		, ; //X6_DESC2
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPINTGER" ) ) ) ;
