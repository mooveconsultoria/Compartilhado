			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If !lAglutina .Or. Len(aProd) == 0
					...
					aadd(aProd,	{Len(aProd)+1,;
								...
								a410Arred( IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(SM0->M0_CODMUN == "3550308",(cAliasSD2)->D2_PRCVEN * (cAliasSD2)->D2_QUANT,(cAliasSD2)->D2_TOTAL),0)+IIf(SM0->M0_CODMUN == "4205407",0,((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR)), "D2_TOTAL"),; //Valor Total
...
static function assina( aDeduz, aNota, aProd, aTotal, aDest )
	...
	If !Empty(SF3->F3_TRIBMUN)	
		cAssinatura += allTrim( strZero( val( SF3->F3_TRIBMUN ), 10 ) )
	Else
...
static function ativ( aProd, aISSQN )
	...
	if !empty( allTrim( aProd[1][19] ) ) 
		...
		If !Empty(SF3->F3_TRIBMUN)
			cString	+= "<codigo>" + allTrim( SF3->F3_TRIBMUN ) + "</codigo>"
		Else
...
Static Function tomador( aDest )
	...
	If !lSemTomador 		
		...
		EndIf
		If Len( cCodTom ) <= 5 .And. (!(cCodTom $ '9999999').Or. (cCodTom $ '9999999' .And. SM0->M0_CODMUN $ '3550308|4205407|3300704|3156700|4115200'))
...
static function infCompl( cMensCli, cMensFis, lNFeDesc, cDescrNFSe )
	...
	cString += "<observacao>" + cMensFis + "</observacao>"
	cString	+= "</infcompl>"
